...
detection_matrix  <- readRDS(gzcon(url("https://github.com/tgelmi-candusso/cameratrap_analysis/raw/main/detection_matrix_revsites_28072022.rds")))
...
detection_matrix$deer$sum_of_rows_na <- rowSums( !is.na(detection_matrix$deer[,1:53]))
detection_matrix$deer$sum_of_rows <- rowSums(detection_matrix$deer[,2:54], na.rm=TRUE)

detected_deer <- detection_matrix$deer %>%
  dplyr::filter(sum_of_rows_na >= 4) %>% dplyr::filter(site_name %in% rev_sites) 

# making it so that the 
has_deer_sites <- detected_deer %>% dplyr::filter(site_name %in% rev_sites)
rev_sites <- has_deer_sites$site_name
...
y <- detected_deer %>%
  select(2:54) ## select final columns select(2:54), this could also be written select(-1) to avoid the first column with the site names

siteCovs <- siteCovs_1000 ### change scale here
...
model_list <- modelList_1000 ##change scale here
models <- get.models(model_list, subset = delta < 2)

site_cov <-siteCovs_1000 ##change scale here
#####
...
occu_modavg_psi_predict_df <- occu_modavg_psi_predict_df %>%
  mutate(x = rep(1:30, 1), # Adding pseudo-coordinates, these could eventually have the actual covariates 
         y = rep(1:30, each = 1))
...
newdata$total_freq_humans <- nseq(site_cov$total_freq_humans, nrow(newdata)) #HERE change covariate name after $
...
occu_pred_plot_fdec <- ggplot(occu_pred_df, aes(x = total_freq_humans, y = Predicted)) +
...
newdata <- as.data.frame(lapply(lapply(site_cov, mean), rep, 25))
###change input HERE based on the covariate of interest after the $ ####
newdata$Fdec_PA <- nseq(site_cov$Fdec_PA, nrow(newdata)) #HERE change covariate name after $
...
##to create the plot for different covariates change the x = 
occu_pred_plot_wvf <- ggplot(occu_pred_df, aes(x = Fdec_PA, y = Predicted)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.5, linetype = "dashed") +
  geom_path(size = 1) +
  theme_classic() +
  coord_cartesian(ylim = c(0,1)) 
occu_pred_plot_wvf + 
  labs(x = "FDEC PA", y = "Occupancy probability")
